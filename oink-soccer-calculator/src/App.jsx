...
export default function OinkSoccerCalc() {
  ...
  const [mySquad, setMySquad] = useState([]); // Full roster
  const [myTeam, setMyTeam] = useState([]); // Active 5
  const [opponentTeam, setOpponentTeam] = useState([]);
  const [myForm, setMyForm] = useState('Pyramid');
  const [oppForm, setOppForm] = useState('Pyramid');

  const [myBoost, setMyBoost] = useState('None');
  const [myBoostApps, setMyBoostApps] = useState(1);
  const [homeAdvantage, setHomeAdvantage] = useState('home'); // 'home' or 'away'
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const docRef = doc(db, 'artifacts', appId, 'users', user.uid, 'soccer_data', 'teams');
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data();
          const loadedSquad = data.mySquad || initialMyTeam;
          setMySquad(loadedSquad);
          setMyTeam(data.myTeam || loadedSquad.slice(0, 5));
          setOpponentTeam(data.opponentTeam || initialOpponent);
          setMyForm(data.myForm || 'Pyramid');
          setOppForm(data.oppForm || 'Pyramid');
          setMyBoost(data.myBoost || 'None');
          setMyBoostApps(data.myBoostApps || 1);
          setHomeAdvantage(data.homeAdvantage || 'home');
        } else {
          setMySquad(initialMyTeam);
          setMyTeam(initialMyTeam.slice(0, 5));
          setOpponentTeam(initialOpponent);
        }
      } catch (error) {
  ...
  }, [myStats, oppStats, homeAdvantage]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center text-slate-400 flex-col gap-4" >
        <Loader2 className="animate-spin w-8 h-8 text-green-500" />
        <p>Loading your scout report...</p>
      </div>
    )
  }
...
function PlayerRow({ player, onEdit, onDelete, isEditing, onInjuryChange, isLast, isActive, isBench, onSwap }) {
